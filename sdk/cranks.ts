...
export async function calcClearingPriceCrank(
  ...
  while (true) {
    try {
      ...
      tx.add(
        genInstr.calculateClearingPrice(
          { limit: new BN(limit) },
          { ...auctionObj }
        )
      );
...
// Returns false if too many errors occured.
export async function matchOrdersCrank(
  provider: anchor.Provider,
  wallet: anchor.Wallet,
  auctionObj: Auction,
  limit: number = 1
  ...
  while (true) {
    try {
      let tx = new anchor.web3.Transaction();
      tx.add(genInstr.matchOrders({ limit: new BN(limit) }, { ...auctionObj }));
      await provider.send(tx, [], { skipPreflight: true });
      numErrors = 0;
...
export async function decryptOrdersCrank(
  ...
  for (let user of fetchedUsers) {
    ...
    let tx = new anchor.web3.Transaction();
    tx.add(genInstr.decryptOrder({ sharedKey }, { ...user, ...auctionObj }));
    ...
    if (numDecryptedUsers % batchSize == 0) {
      ...
      console.log(
        tempDecryptInstrs.length,
        "open order accounts decrypted in",
        (Date.now() - startTime) / 1000
      );
...
export async function consumeEventsCrank(
  provider: anchor.Provider,
  auctionObj: Auction,
  numEventsToConsume: number = 10
): Promise<boolean> {
  ...
  while (true) {
    ...
    numEventsToConsume = Math.min(
      numEventsToConsume,
      thisEventQueue.header.count.toNumber()
    );
    ...
    for (let thisPubkeyStr of pubkeyStrs) {
      remainingAccounts.push({
        pubkey: new PublicKey(thisPubkeyStr),
        isSigner: false,
        isWritable: true,
      });
    }
...
// Returns false if any errors occured but will keep running the crank until all accounts are processed.
export async function settleAndCloseOpenOrdersCrank(
  program: anchor.Program<AuctionHouse>,
  provider: anchor.Provider,
  auctionObj: Auction,
  batchSize: number = 50
  ...
  for (let user of fetchedUsers) {
    let tx = new anchor.web3.Transaction();
    tx.add(genInstr.settleAndCloseOpenOrders({ ...auctionObj, ...user }));
    ...
    if (numOpenOrdersClosed % batchSize == 0) {
      ...
      console.log(
        tempCloseInstrs.length,
        "open order accounts settled and closed in",
        (Date.now() - startTime) / 1000
      );
