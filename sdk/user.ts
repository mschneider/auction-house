...
export async function fetchUsers(
  ...
  provider: anchor.Provider,
  auction: Auction,
  opts?: { onlyEncrypted?: boolean; onlyEmpty?: boolean }
): Promise<Array<User>> {
  ...
  for (let thisAccount of programAccounts) {
    let thisOpenOrders = genAccs.OpenOrders.decode(
      thisAccount.account.data as Buffer
    );
    ...
    let [orderHistory] = await anchor.web3.PublicKey.findProgramAddress(
      [
        thisOpenOrders.authority.toBuffer(),
        Buffer.from("order_history"),
        Buffer.from(auction.auctionId),
        auction.auctioneer.toBuffer(),
      ],
      program.programId
    );
    let thisUser: User = {
      ...
      orderHistory,
      userBase: await getAssociatedTokenAddress(
        auction.baseMint,
        thisOpenOrders.authority
      ),
      userQuote: await getAssociatedTokenAddress(
        auction.quoteMint,
        thisOpenOrders.authority
      ),
    };
