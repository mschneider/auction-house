...
describe("auction-house", () => {
  ...
  it("inits the auction", async () => {
    ...
    tx.add(genInstr.initAuction({ args: { ...auction } }, { ...auction }));
  ...
  it("init open orders", async () => {
    ...
    assert.isTrue(
      bidOpenOrders.authority.toString() == thisBidUser.user.toString(),
      "check bid open orders init correctly"
    );
  });
  ...
  it("cancels an order", async () => {
    ...
    tx.add(genInstr.cancelOrder({ orderId }, { ...thisAskUser, ...auction }));
    await provider.send(tx, [thisAskUser.userKeypair], { skipPreflight: true });
    thisOpenOrders = await genAccs.OpenOrders.fetch(
      provider.connection,
      thisAskUser.openOrders
    );
    console.log(
      await provider.connection.getTokenAccountBalance(thisAskUser.userBase)
    );
  ...
  it("new encrypted order", async () => {
    ...
    assert.isTrue(
      thisOpenOrders.numOrders == 2,
      "check the orders have been added"
    );
  });
  ...
  it("decrypts order", async () => {
    ...
    let tx = new anchor.web3.Transaction();
    tx.add(
      genInstr.decryptOrder({ sharedKey }, { ...thisBidUser, ...auction })
    );
    ...
    assert.isTrue(
      ...
      "check that the encrypted orders have been cleared out"
    );
    assert.isTrue(
      thisOpenOrders.orders.length == 1,
      "check the order has been added to the order book"
    );
  });

  it("calculates the clearing price", async () => {
    let remainingTimeToClearing =
      auction.endDecryptionPhase - Date.now() / 1000;
    if (remainingTimeToClearing > 0) {
      await sleep(remainingTimeToClearing + 1);
    }
    let tx = new anchor.web3.Transaction();
    tx.add(
      genInstr.calculateClearingPrice({ limit: new BN(10) }, { ...auction })
    );
    await provider.send(tx, [], { skipPreflight: true });
    let thisAuction = await genAccs.Auction.fetch(
      provider.connection,
    ...
    assert.isTrue(
      ...
      "Auction has found clearing price"
    );
  });

  it("matches the price", async () => {
    let tx = new anchor.web3.Transaction();
    tx.add(genInstr.matchOrders({ limit: new BN(10) }, { ...auction }));
    tx.add(genInstr.matchOrders({ limit: new BN(10) }, { ...auction }));
    await provider.send(tx, [], { skipPreflight: true });
    let thisAuction = await genAccs.Auction.fetch(
      provider.connection,
    ...
    assert.isTrue(
      thisAuction.remainingBidFills.eq(new BN(0)),
      "Bid orders filled"
    );
    assert.isTrue(
      thisAuction.remainingAskFills.eq(new BN(0)),
      "Ask orders filled"
    );
  });
  ...
  it("settle and closes open orders", async () => {
    let tx = new anchor.web3.Transaction();
    tx.add(genInstr.settleAndCloseOpenOrders({ ...auction, ...users[0] }));
    tx.add(genInstr.settleAndCloseOpenOrders({ ...auction, ...users[1] }));
    await provider.send(tx, [], { skipPreflight: true });
    console.log(
      await provider.connection.getTokenAccountBalance(users[0].userBase)
    );
    console.log(
      await provider.connection.getTokenAccountBalance(users[0].userQuote)
    );
    console.log(
      await provider.connection.getTokenAccountBalance(users[1].userBase)
    );
    console.log(
      await provider.connection.getTokenAccountBalance(users[1].userQuote)
    );
  });
  ...
  it("close the AOB accounts and retrieve rent", async () => {
    ...
    let tx = new anchor.web3.Transaction();
    tx.add(genInstr.closeAobAccounts({ ...auction }));
    await provider.send(tx, [], { skipPreflight: true });
    let solAfter = await provider.connection.getBalance(wallet.publicKey);
    assert.isTrue(
      solAfter > solBefore,
      "checking the Sol balance has increase from reclaiming rent"
    );
  });
});
